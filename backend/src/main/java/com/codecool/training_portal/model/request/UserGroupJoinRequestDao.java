...
public interface UserGroupJoinRequestDao extends JpaRepository<UserGroupJoinRequest, Long> {
    @Query(
            "SELECT ugjr FROM UserGroupJoinRequest ugjr WHERE ugjr.id = :id AND ugjr.userGroup.id = :groupId")
    ...
    Optional<UserGroupJoinRequest> findByIdAndApplicationUser(Long id, ApplicationUser applicationUser);
